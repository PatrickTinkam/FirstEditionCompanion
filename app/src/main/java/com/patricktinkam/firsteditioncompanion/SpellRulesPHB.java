...
public final class SpellRulesPHB {
  ...
  static {
    ...
    D.put("slow","Reduces affected creatures' movement and attack rates; it counters haste. Exact target limits and saving throw handling use the spell's listed metadata.");
  ...
  static String digestFor(String name){String d=D.get(norm(name));if(d!=null)return d;String k=norm(name);if(k.startsWith("detect"))return "Detects the named presence, direction, nature, or condition within the spell's listed range and duration.";if(k.startsWith("cure"))return "Removes or repairs the named injury or affliction subject to the spell's listed target, range, and saving-throw rules.";if(k.startsWith("protectionfrom")||k.startsWith("resist"))return "Provides the named magical protection or resistance under the listed duration and area, with exact modifiers determined by the spell version.";if(k.startsWith("monstersummoning")||k.startsWith("animalsummoning"))return "Summons creatures appropriate to the spell level according to the listed arrival and duration rules.";if(k.startsWith("wallof"))return "Creates the named magical barrier with the listed dimensions and duration, using the barrier's particular passage, damage, and destruction rules.";return "";}
